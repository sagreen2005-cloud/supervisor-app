...
async function downloadRollCallPdf(rollCallId) {
  ...
  const branding =
    typeof getDepartmentBranding === "function"
      ? await getDepartmentBranding()
      : null;

  const html = buildRollCallPrintHtml(rollCall, branding);
...
function buildRollCallPrintHtml(rollCall, branding) {
  const useBranding = Boolean(
    branding &&
    branding.useOnRollCall &&
    typeof buildDepartmentLetterheadHtml === "function"
  );

  const letterhead = useBranding
    ? buildDepartmentLetterheadHtml(branding, {
        documentTitle: "Supervisor Roll Call"
      })
    : `
  ...
  const departmentCss =
    typeof getDepartmentDocumentCss === "function"
